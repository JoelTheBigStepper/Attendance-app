...
async function processAttendanceJob(job) {
...
// Connect to MongoDB and start processing queue
(async () => {
  try {
    await mongoose.connect(process.env.MONGO_URI);
    console.log("✅ MongoDB connected. Attendance worker running...");

    attendanceQueue.process(async (job) => {
      try {
        await processAttendanceJob(job);
        console.log(`✅ Attendance processed for matric: ${job.data.matric}`);
      } catch (err) {
        console.error(`❌ Attendance job failed for matric ${job.data.matric}:`, err.message);
        // Optionally, you can fail the job to retry later or discard
        throw err;
      }
    });

    attendanceQueue.on("failed", (job, err) => {
      console.error(`Job failed for matric ${job.data.matric}:`, err.message);
    });
  } catch (err) {
